...
const TokenPln = () => {
  // State Form (Frontend pakai CamelCase biar rapi di JS)
  const [form, setForm] = useState({
    idPel: '', namaPel: '', tarif: 'R1', daya: '1300',
    nominal: '20000', admin: '3000', tglManual: ''
  });
  
  // State Toko Default (Mencegah undefined)
  ...
  const doPrint = (dataPrint) => {
    try {
        // Fallback: Jika dataPrint tidak bawa info toko, pakai state 'toko'
        const infoToko = dataPrint.toko || toko;
        
        const namatoko = infoToko.nama_toko || "NAMA TOKO";
        ...
        doc.write(`
          <html>
            ...
            <body>
              ...
              <div class="row"><span>Tgl:</span><span class="r">${dataPrint.tgl}</span></div>
        ...
        // Print Delay (Penting agar font terload)
        setTimeout(() => {
  ...
  const handleReprint = (item) => {
    try {
        if (!item.nama_produk) return alert("Data struk rusak.");
        const detail = JSON.parse(item.nama_produk);
        
        // Cek apakah data lama atau baru
        const nominalFix = detail.nominal_asli ? detail.nominal_asli : (parseInt(item.total_belanja) - 3000);
        
        doPrint({
            toko: toko, // Kirim data toko saat ini untuk cetak ulang
        ...
        alert("Gagal load data struk lama.");
    }
  };
  ...
  const handleProses = (e) => {
    ...
    // MAPPING DATA (KUNCI PERBAIKAN ERROR PHP)
    // Backend minta 'id_pelanggan', kita punya 'idPel'
    const payload = {
    ...
    axios.post(`${BASE_URL}/api_digital.php`, payload)
      .then(res => {
        ...
        if(res.data.success) {
            ...
            loadData();
            // Reset form (kecuali admin & tanggal)
                ...
                <tbody className="divide-y divide-slate-700">
                    {riwayat.map((item, i) => (
                        <tr key={i} className="hover:bg-slate-700/30 transition">
                            <td className="p-3 whitespace-nowrap">{item.tgl_transaksi}</td>
